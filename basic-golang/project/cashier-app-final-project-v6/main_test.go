		...
		Describe("Sessions repository", func() {
			When("add session data to file sessions.json", func() {
				It("should save data session as a list to file sessions.json", func() {
					...
					token, err := sessionRepo.TokenExist("cc03dbea-4085-47ba-86fe-020f5d01a9d8")
					Expect(err).ShouldNot(HaveOccurred())
		...
		Describe("Cart repository", func() {
			When("add cart data to file carts.json", func() {
				It("should save data cart to file carts.json", func() {
					...
					err := cartRepo.AddCart(productCart[0])
					Expect(err).ShouldNot(HaveOccurred())

					actualCart, err := cartRepo.ReadCart()
					Expect(err).ShouldNot(HaveOccurred())
					Expect(productCart).To(Equal(actualCart))

					cartRepo.ResetCarts()
				})
			})

			When("update cart data from file carts.json", func() {
				It("should updated data cart from file carts.json", func() {
					productCart := []model.Cart{{
						Name: "aditira",
						Cart: []model.Product{
							{Id: "1", Name: "Tea", Price: "20000", Quantity: "1", Total: 20000},
					...
					cartUpdate := []model.Cart{{
						Name: "aditira",
						Cart: []model.Product{
							{Id: "2", Name: "Juice", Price: "5000", Quantity: "2", Total: 10000},
							{Id: "5", Name: "Cake", Price: "35000", Quantity: "1", Total: 35000},
						},
						TotalPrice: 45000,
					}}

					err = cartRepo.UpdateCart(cartUpdate[0])
					Expect(err).ShouldNot(HaveOccurred())

					actualCart, err := cartRepo.ReadCart()
					Expect(err).ShouldNot(HaveOccurred())
					Expect(cartUpdate).To(Equal(actualCart))
		...
		Describe("/cart/add", func() {
			...
			When("add 1 product to cart with POST method and login cookie", func() {
				It("should added 1 cart data to file carts.json with `quantity per-product`, `price per-product` and `total price` calculated", func() {
					...
					//add 1 product to cart
					w = httptest.NewRecorder()
					r = httptest.NewRequest(http.MethodPost, "/cart/add", strings.NewReader("product=6,Cake,40000,2"))
					r.Header.Set("Content-Type", "application/x-www-form-urlencoded")
					r.AddCookie(cookie)
					server.Handler().ServeHTTP(w, r)
					...
					expectCart := []model.Cart{{
						Name: "aditira",
						Cart: []model.Product{
							{Id: "6", Name: "Cake", Price: "40000", Quantity: "2", Total: 80000},
						},
						TotalPrice: 80000,
					}}
					Expect(cart).To(Equal(expectCart))
