...
func (t Teacher) Join(db *gorm.DB) ([]Joined, error) {
	var joined []Joined	
	if err := db.Table("teachers").Select("teachers.name as teacher_name, schools.name as school_name, classes.name as class_name, lessons.name as lesson_name").Joins("JOIN lessons ON teachers.lesson_id = lessons.id").Joins("JOIN classes ON teachers.class_id = classes.id").Joins("JOIN schools ON teachers.school_id = schools.id").Find(&joined).Error; err != nil {
		return []Joined{}, err 
	}
